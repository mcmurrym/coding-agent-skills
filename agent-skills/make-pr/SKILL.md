...
After a PR is created successfully and the Codex inline visualization surface and `$visualize` skill are available, load and follow `$visualize`. Write a compact HTML fragment to the task-owned visualization directory outside the repository, emit its `visualize{"path":"<absolute-path>"}` content reference in the same response, and do not print raw HTML in chat. Include a native **GH PR Comment Audit** button, follow the visualization design system, and call `window.openai.sendFollowUpMessage({ prompt, title })` from an event listener. Keep the exact created PR URL, repository, and PR number in the published response or fragment data, not in the button prompt. Resolve symlinks for `$gh-pr-comment-audit`, prefer its canonical repository-source `SKILL.md` path, and send exactly `[$gh-pr-comment-audit](<absolute-gh-pr-comment-audit-skill-path>)` as the complete follow-up prompt. Do not add punctuation, PR metadata, audit instructions, or any other text. Do not use an inline `onclick`, network request, or `request_user_input`, and do not start the audit merely because the button was displayed.
